...
void main( void ) {
	...
	vec2 refractUv = gl_FragCoord.xy / winResolution.xy;
	float slide;
	vec2 refractUvR;
	vec2 refractUvG;
	vec2 refractUvB;
	float refractPower = 0.5;
	...
	for ( int i = 0; i < 16; i ++ ) {
		...
		slide = float( UNROLLED_LOOP_INDEX ) / 16.0 * 0.075 ;//* random( screenUv );

		refractUvR = refractUv - (geo.normal.xy, geo.normal.xy) * ( refractPower + slide * 1.0 );
		refractUvG = refractUv - (geo.normal.xy, geo.normal.xy) * ( refractPower + slide * 1.5 );
		refractUvB = refractUv - (geo.normal.xy, geo.normal.xy) * ( refractPower + slide * 2.0 );
